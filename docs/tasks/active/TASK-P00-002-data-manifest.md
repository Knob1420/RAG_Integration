...
1. 归档内的 1 个 zip 是否需要展开并入清单（若展开，条数将超过 83，需更新验收口径）；
2. `safety_level` 83 条全部 UNKNOWN，待用户逐份/按目录批量定级（P00 后续任务）；
3. 仓库内 `data/` 存放原始资料的方案是否长期接受，或后续迁至仓库外（涉及 source_id 稳定性，越早决定越好）。
